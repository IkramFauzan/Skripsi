...
import com.example.testingfirebaserealtime.databinding.ActivityDetailSejarahBinding
...
class DetailSejarahActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btn.setOnClickListener {
            ...
            if (message == 0f && review.isEmpty()) {
                Toast.makeText(this@DetailSejarahActivity, "Rating dan komentar harus diisi", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (message == 0f) {
                Toast.makeText(this@DetailSejarahActivity, "Anda harus memilih rating sebelum memberi komentar", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (review.isEmpty()) {
                Toast.makeText(this@DetailSejarahActivity, "Anda harus mengisi field sebelum memberi komentar", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            if (firebaseUser != null) {
                            ...
                            ratingRef.setValue(ulasan)
                                .addOnCompleteListener { task ->
                                    if (task.isSuccessful) {
                                        Toast.makeText(this@DetailSejarahActivity, "Rating added successfully!", Toast.LENGTH_SHORT).show()
                                    } else {
                                        Toast.makeText(this@DetailSejarahActivity, "Failed to add rating!", Toast.LENGTH_SHORT).show()
                                    }
                                }
                        } else {
                            Toast.makeText(this@DetailSejarahActivity, "User data not found!", Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onCancelled(error: DatabaseError) {
                        Toast.makeText(this@DetailSejarahActivity, "Failed to fetch user data: ${error.message}", Toast.LENGTH_SHORT).show()
                    }
                })
            } else {
                ...
                alertBuilder.setPositiveButton("OK") {dialog, which ->
                    val intent = Intent(this@DetailSejarahActivity, SignActivity::class.java)
                        ...
                        favoriteRef.orderByChild("placeName").equalTo(place).addListenerForSingleValueEvent(object :
                            ...
                            override fun onDataChange(dataSnapshot: DataSnapshot) {
                                if (dataSnapshot.exists()) {
                                    ...
                                    //Toast.makeText(this@DetailSejarahActivity, "Tempat sudah ditambahkan ke favorit", Toast.LENGTH_SHORT).show()
                                }
//
                            }

                            override fun onCancelled(error: DatabaseError) {
                                Toast.makeText(this@DetailSejarahActivity, "Gagal memeriksa tempat favorit: ${error.message}", Toast.LENGTH_SHORT).show()
                            }
                        })
                    } else {
                        Toast.makeText(this@DetailSejarahActivity, "Informasi pengguna tidak ditemukan", Toast.LENGTH_SHORT).show()
                    }

                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@DetailSejarahActivity, "Gagal mengambil informasi pengguna: ${error.message}", Toast.LENGTH_SHORT).show()
                }
            })
        } else {
            Toast.makeText(this@DetailSejarahActivity, "Informasi tempat tidak lengkap", Toast.LENGTH_SHORT).show()
        }
    ...
    private fun addFavoritePlace() {
        ...
        val placeName = intent.getStringExtra(DetailSejarahActivity.PLACE)
        val username = currentUser?.displayName // Mengambil nama pengguna
        val placeAddress = intent.getStringExtra(DetailSejarahActivity.ADDRESS)
        val placePhotoUrl = intent.getStringExtra(DetailSejarahActivity.PHOTO)
