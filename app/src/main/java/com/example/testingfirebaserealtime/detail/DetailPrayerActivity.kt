...
import com.example.testingfirebaserealtime.databinding.ActivityDetailPrayerBinding
...
class DetailPrayerActivity : AppCompatActivity(), OnMapReadyCallback {

    private lateinit var binding: ActivityDetailPrayerBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityDetailPrayerBinding.inflate(layoutInflater)
        ...
        binding.btn.setOnClickListener {
            ...
            if (message == 0f && review.isEmpty()) {
                Toast.makeText(this@DetailPrayerActivity, "Rating dan komentar harus diisi", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (message == 0f) {
                Toast.makeText(this@DetailPrayerActivity, "Anda harus memilih rating sebelum memberi komentar", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (review.isEmpty()) {
                Toast.makeText(this@DetailPrayerActivity, "Anda harus mengisi field sebelum memberi komentar", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            if (firebaseUser != null) {
                            ...
                            ratingRef.setValue(ulasan)
                                .addOnCompleteListener { task ->
                                    if (task.isSuccessful) {
                                        Toast.makeText(this@DetailPrayerActivity, "Rating added successfully!", Toast.LENGTH_SHORT).show()
                                    } else {
                                        Toast.makeText(this@DetailPrayerActivity, "Failed to add rating!", Toast.LENGTH_SHORT).show()
                                    }
                                }
                        } else {
                            Toast.makeText(this@DetailPrayerActivity, "User data not found!", Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onCancelled(error: DatabaseError) {
                        Toast.makeText(this@DetailPrayerActivity, "Failed to fetch user data: ${error.message}", Toast.LENGTH_SHORT).show()
                    }
                })
            } else {
                ...
                alertBuilder.setPositiveButton("OK") {dialog, which ->
                    val intent = Intent(this@DetailPrayerActivity, SignActivity::class.java)
...
//
                            ...
                            override fun onCancelled(error: DatabaseError) {
                                Toast.makeText(this@DetailPrayerActivity, "Gagal memeriksa tempat favorit: ${error.message}", Toast.LENGTH_SHORT).show()
                            }
                        })
                    } else {
                        Toast.makeText(this@DetailPrayerActivity, "Informasi pengguna tidak ditemukan", Toast.LENGTH_SHORT).show()
                    }

                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@DetailPrayerActivity, "Gagal mengambil informasi pengguna: ${error.message}", Toast.LENGTH_SHORT).show()
                }
            })
        } else {
            Toast.makeText(this@DetailPrayerActivity, "Informasi tempat tidak lengkap", Toast.LENGTH_SHORT).show()
        }
