...
import com.example.testingfirebaserealtime.databinding.ActivityDetailBelanjaBinding
...
class DetailWisataBuatanActivity : AppCompatActivity(), OnMapReadyCallback {

    private lateinit var binding: ActivityDetailBelanjaBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityDetailBelanjaBinding.inflate(layoutInflater)
        ...
        // Ensure your RatingBar is set to 0 by default if no rating is selected
        ...
        binding.btn.setOnClickListener {
            ...
            if (message == 0f && review.isEmpty()) {
                Toast.makeText(this@DetailWisataBuatanActivity, "Rating dan komentar harus diisi", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (message == 0f) {
                Toast.makeText(this@DetailWisataBuatanActivity, "Anda harus memilih rating sebelum memberi komentar", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (review.isEmpty()) {
                Toast.makeText(this@DetailWisataBuatanActivity, "Anda harus mengisi field sebelum memberi komentar", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            if (firebaseUser != null) {
                            ...
                            ratingRef.setValue(ulasan)
                                .addOnCompleteListener { task ->
                                    if (task.isSuccessful) {
                                        Toast.makeText(this@DetailWisataBuatanActivity, "Rating added successfully!", Toast.LENGTH_SHORT).show()
                                    } else {
                                        Toast.makeText(this@DetailWisataBuatanActivity, "Failed to add rating!", Toast.LENGTH_SHORT).show()
                                    }
                                }
                        } else {
                            Toast.makeText(this@DetailWisataBuatanActivity, "User data not found!", Toast.LENGTH_SHORT).show()
                        }
                    }

                    override fun onCancelled(error: DatabaseError) {
                        Toast.makeText(this@DetailWisataBuatanActivity, "Failed to fetch user data: ${error.message}", Toast.LENGTH_SHORT).show()
                    }
                })
            } else {
                ...
                alertBuilder.setPositiveButton("OK") {dialog, which ->
                    val intent = Intent(this@DetailWisataBuatanActivity, SignActivity::class.java)
                        ...
                        favoriteRef.orderByChild("placeName").equalTo(place).addListenerForSingleValueEvent(object :
                            ...
                            override fun onDataChange(dataSnapshot: DataSnapshot) {
                                if (dataSnapshot.exists()) {
                                    ...
                                    //Toast.makeText(this@DetailBelanjaActivity, "Tempat sudah ditambahkan ke favorit", Toast.LENGTH_SHORT).show()
                                }
//
                            }

                            override fun onCancelled(error: DatabaseError) {
                                Toast.makeText(this@DetailWisataBuatanActivity, "Gagal memeriksa tempat favorit: ${error.message}", Toast.LENGTH_SHORT).show()
                            }
                        })
                    } else {
                        Toast.makeText(this@DetailWisataBuatanActivity, "Informasi pengguna tidak ditemukan", Toast.LENGTH_SHORT).show()
                    }

                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@DetailWisataBuatanActivity, "Gagal mengambil informasi pengguna: ${error.message}", Toast.LENGTH_SHORT).show()
                }
            })
        } else {
            Toast.makeText(this@DetailWisataBuatanActivity, "Informasi tempat tidak lengkap", Toast.LENGTH_SHORT).show()
        }
    ...
    private fun addFavoritePlace() {
        ...
        val placeName = intent.getStringExtra(DetailWisataBuatanActivity.PLACE)
        val username = currentUser?.displayName // Mengambil nama pengguna
        val placeAddress = intent.getStringExtra(DetailWisataBuatanActivity.ADDRESS)
        val placePhotoUrl = intent.getStringExtra(DetailWisataBuatanActivity.PHOTO)
                                        ...
                                        placeSnapshot.ref.removeValue()
                                            .addOnCompleteListener { task ->
                                                if (task.isSuccessful) {
                                                    ...
                                                    setupFavoriteButtonAppearance(isFavorite)
                                                    Toast.makeText(
                                                        this@DetailWisataBuatanActivity,
